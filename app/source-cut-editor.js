(function () {
  ...
  const communityIds = ['communityBrand','communityTitle','communityBody','communityComments','commentPaddingTop','commentPaddingBottom'];
  let currentStep = 1;
  ...
  function syncCommunityUi() {
    ...
    for (const id of communityIds) {
      ...
      const map = {
        communityBrand:'brand', communityTitle:'title', communityBody:'body',
  ...
  function syncExport() { const communityBlocked = project.community?.enabled && !String(project.community.body || '').trim(); $('exportZip').disabled = busy || !previewReady || !project.complete || communityBlocked; }
  ...
  function preview() {
    ...
    if (project.community?.enabled && !String(project.community.body || '').trim()) { $('previewError').textContent = '새 커뮤니티 양식을 쓰려면 오른쪽에 원문 본문 텍스트를 입력하세요. 원본 사이트 화면을 대신 출력하지 않습니다.'; }
