(function () {
  ...
  function normalizeAsset(asset, index) {
    ...
    return {
      ...
      kind,
      acquisitionState,
      sourceWidth: positiveInt(asset.sourceWidth),
      sourceHeight: positiveInt(asset.sourceHeight),
      provenance: cleanText(asset.provenance, 500),
      captureUrl: cleanText(asset.captureUrl, 1200),
      observedAt: cleanText(asset.observedAt, 80),
  ...
  function build(input) {
    ...
    for (let i = 1; i < bodyAssets.length; i += 1) {
      if (bodyAssets[i].sourceSequence < bodyAssets[i - 1].sourceSequence) {
        throw new Error('original post screenshots/media must preserve sourceSequence order');
      }
    }
    ...
    return {
      ...
      bodyAssetsAcquired,
      assetsPending: fullBodyCaptureStatus !== 'complete' || !bodyAssetsAcquired,
