# 룸메가 병신같은 후기

- source: TheQoo 후기
- source_url: https://theqoo.net/review/242181037
- exact_observed_title: `룸메가 병신같은 후기`
- observed_at: 2026-09-23 00:14 KST
- provenance_level: C1
- acquisition: public individual page discovered/read through public web index; exact individual public URL verified
- body_read: true
- comments_read: false
- image_or_screenshot_present: false / 이번 발굴에서 별도 이미지·스크린샷 확인·다운로드 안 함
- observed_metrics: 조회 수 1482, 댓글 5 (공개 페이지에 관찰된 값만 기록; 댓글 내용은 읽지 않음)
- why_good: 룸메이트가 규정과 달리 남자친구를 데려와 함께 살고, 밤 소음·전기세·공용 주방 사용 문제까지 겹치는 생활 갈등이다. 공동생활에서 어디까지 참아야 하는지 바로 의견이 나오는 상황이다.
- claim_status: 작성자의 개인 경험 주장으로 미검증
- rights_status: 미확인
- OCR: false
- moderation_status: 미판정
- asset_status: A0
- publication_status: P0
- A1: false
- P1: false
- publicationAllowed: false
