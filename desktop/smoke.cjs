...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const community = await editor.webContents.executeJavaScript(`(async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 350));
      return {
        enabled:p.community.enabled, title:p.community.title, body:p.community.body, comments:p.community.comments,
        kinds:slides.map(s=>s.type), commentKinds:slides.map(s=>s.communityPage?.kind).filter(Boolean),
        step3:document.querySelector('[data-step="3"]').classList.contains('active'),
    ...
    assert.equal(community.step3,true);
    assert.equal(community.previewError,'');
    assert(community.previewCanvases >= 3);
    console.log('SMOKE community preview step PASS');
