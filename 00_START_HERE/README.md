...
> **정기/스케줄 실행은 먼저 [`NEXT_RUN_HANDOFF.md`](./NEXT_RUN_HANDOFF.md)를 읽고 현재 `main`을 확인한 뒤, 계획만 보고하지 말고 남은 구현을 실제로 진행한다.**
...
1. 스케줄/정기 실행이면 [`NEXT_RUN_HANDOFF.md`](./NEXT_RUN_HANDOFF.md)를 먼저 읽고 최신 `main`/최근 커밋을 확인한다.
2. 내가 지금 어느 역할인지 확인한다.
3. 해당 역할 폴더의 README를 먼저 읽는다.
4. 필요한 경우 위 벤치마크/Playbook/Matrix를 확인한다.
5. 입력 계약을 확인한다.
6. 자기 역할의 산출물만 만든다.
7. 완료 후 다음 역할의 입력 형식으로 넘긴다.
8. 의미 있는 변경 후 operations hub `T-0008-ai-content-monetization`에 다음 실행자가 바로 이어갈 수 있는 인수인계를 남긴다.
