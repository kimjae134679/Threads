...
for (const file of files) {
  ...
  // C0 is index/list-only by definition. Never place it in deterministic screenshot capture,
  // even if a URL-like string appears in the record; first verify the exact individual source and promote to C1.
  if (c === 'C0') {
    provenancePending.push({ file, title, source, url });
    continue;
  }
  ...
  const score = (koreanSource(source) ? 1000 : 0) + (c === 'C1' ? 100 : 90) + (url ? 20 : 0) + (fullBody ? 10 : 0) + (comments ? 2 : 0) + (assetsPending ? 1 : 0);
...
const lines = [
  ...
  `Generated from one-candidate-per-file records in \`${candidatesDir}\`. Deterministic capture queue: **${selected.length}** / eligible exact-source **${rows.length}** / provenance-pending C0 **${provenancePending.length}** / blocked inconsistent **${blocked.length}**.`,
  'Korean-community exact-source candidates are ranked before overseas support lanes. C0/index-only records are never emitted into deterministic capture order.', '',
  '## Capture order', ''
];
selected.forEach((r, i) => {
  ...
  lines.push(`- Exact public URL: ${r.url || '미확인'}`);
...
if (provenancePending.length) {
  lines.push('## Provenance pending — not capture-ready', '');
  provenancePending.forEach(r => lines.push(`- \`${r.file}\` — C0/index-only; verify exact individual public source first, then promote to C1 before screenshot intake.`));
  lines.push('');
}
if (blocked.length) {
...
lines.push('## Fail-closed rules', '', '- Never fabricate missing screenshots or body cards.', '- Never infer capture/OCR/moderation/rights success from this queue.', '- Exact URL is read only from the candidate `## 정확한 링크` section; unrelated URLs elsewhere cannot accidentally upgrade acquisition.', '- C0/index-only leads never enter deterministic screenshot capture; exact individual provenance must be verified and the candidate promoted to C1 first.', '- Privacy masking remains user-directed; no automatic masking.', '- After real bytes exist: deterministic intake → human completeness verification → UI-chrome crop review → reviewed-crop validation → 1080×1080 contain normalization → strict carousel validation.', '');
await writeFile(out, lines.join('\n'), 'utf8');
console.log(`Wrote ${selected.length} exact-source acquisition targets to ${out}; held ${provenancePending.length} C0 provenance-pending; blocked ${blocked.length} inconsistent records`);
